...
<html>
  ...
  <body>
    <form method="post" class="form-horizontal" name="optionForm" enctype="multipart/form-data">
    	...
    	<fieldset>
    	<legend><h3>Upload Assignment</h3></legend>
    	<input type="hidden" id="stuid" name="trainerid" value="<%=(String)session.getAttribute("id") %>">
          ...
          <div class="alert alert-success fade in" role="alert">
      		...
      		<strong>Success !</strong> Assignment have been uploaded.
   		  </div>
   		   <%
          ...
          <div class="alert alert-danger fade in" role="alert">
      		<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
      		<strong>Oh Snap !</strong> Unable to upload assignment, please try again.
   		  </div>
   		   <%
   		   session.removeAttribute("up");
   		  }
   		  else if(session.getAttribute("delete")!= null && (Integer)session.getAttribute("delete") == 1)
   		  {
           %>
          <div class="alert alert-success fade in" role="alert">
      		<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		 ...
		 <select name="classid" class="form-control" required>
		 	<%
		 		...
		 		while(res.next())
		 		{
		 			%><option value="<%=res.getString(1)%>"><%=res.getString(2) %> - <%=res.getString(3) %> - <%=res.getString(4) %> - <%=res.getString(5) %></option><%
		 		}
		 	 %>
			    ...
			    <div class="panel-heading">
			      <h4 class="panel-title">
			        <a data-toggle="collapse" data-parent="#accordion" href="#collapse<%=rs.getString(1)%>">
			         <strong> <%=rs.getString(2) %></strong>&nbsp;&nbsp;&nbsp;No. Of Uploads : <strong><font id="numcom"><%=rs.getInt(5) %></font></strong>&nbsp;&nbsp;Schedule :<strong><%=rs.getString(3) %></strong>&nbsp;&nbsp;Center : <strong><%=rs.getString(4) %></strong>
			        </a>
			      </h4>
			    </div>
